...
handleEvent g (VtyEvent (V.EvKey (V.KChar 'J') []))       = continue $ minerChangeBomb g (-1)
handleEvent g (VtyEvent (V.EvKey (V.KChar 'j') []))       = continue $ minerChangeBomb g (-1)
handleEvent g (VtyEvent (V.EvKey (V.KChar 'K') []))       = continue $ minerChangeBomb g 1
handleEvent g (VtyEvent (V.EvKey (V.KChar 'k') []))       = continue $ minerChangeBomb g 1
...
drawUI g =
  [ C.center $ vBox [padRight (Pad 2) (drawStats g) <+> vBox [padRight (Pad 2) $ drawGrid g,  padRight (Pad 2) $ drawBombTypeWidget g]]]
...
drawBombTypeWidget :: Game -> Widget Name
drawBombTypeWidget g = hLimit 47
  $ vBox [ padTop (Pad 2) $ drawBombType (g ^. bombType)]

drawBombType :: Int -> Widget Name
drawBombType n = withBorderStyle BS.unicodeBold
  $ B.borderWithLabel (str "Bomb Type")
  $ C.hCenter
  $ padAll 1
  $ str $ bombTypeMap n

bombTypeMap :: Int -> String
bombTypeMap n
  | n == 0 = "Holy Cross"
  | n == 1 = "Block Maker"
  | n == 2 = "Ring of Truth"
  | otherwise = "Mommy I'm so scared that's a big ass bomb"
